...
- By default, Ollama cache is stored in `~/.ollama` directory, but you can configure it to match your host's Olama cache location (see `HARBOR_OLLAMA_CACHE` in [configuration](#configuration) below)
...
- `compose.x.ollama.rocm.yml` adds AMD ROCm devices (`/dev/kfd`, `/dev/dri`) and switches the `ollama` container to `${HARBOR_OLLAMA_IMAGE}:rocm`. The `ollama-init` sidecar remains on Alpine because it only calls the server's HTTP API.
...
- **ollama-init sidecar** (defined in base `compose.ollama.yml`): depends on `ollama` healthy, uses the server's `/api/pull` endpoint for every model in the comma-separated `HARBOR_OLLAMA_DEFAULT_MODELS` (default `nomic-embed-text:latest`), then stays healthy so `harbor up` with `--wait` succeeds even during long first-time pulls. The Alpine sidecar only mounts its init script. Models are stored through the Ollama server in its cache volume.
...
You'll find many more examples and scenarios covered in the [troubleshooting guide](./1.-Harbor-User-Guide.md#troubleshooting).
...
See environment variables [configuration guide](./1.-Harbor-User-Guide.md#environment-variables).
...
In addition to that, some high-level configurations are available via [`harbor config`](./3.-Harbor-CLI-Reference.md#harbor-config)
...
Follow Harbor's [environment variables guide](./1.-Harbor-User-Guide.md#environment-variables) to set arbitrary variables for `ollama` service.
...
Additionally, you can find a small [HTTP playbook](../services/http-catalog/ollama.http) in the [`services/http-catalog/`](https://github.com/av/harbor/tree/main/services/http-catalog) folder.
...
The path must be either absolute or relative to the [`harbor home`](./3.-Harbor-CLI-Reference.md#harbor-home) directory.
