...
`--mm-processor-kwargs` bounds the pixel budget the vision tower sees; those are the values Chandra was tuned for, not vLLM's defaults. Lower `--gpu-memory-utilization` or `--max-model-len` through `HARBOR_CHANDRA_EXTRA_ARGS` if the model does not fit — the later occurrence wins in vLLM's argument parser.
